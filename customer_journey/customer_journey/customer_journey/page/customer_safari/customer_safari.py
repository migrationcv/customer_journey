...
def prepare_data_dict(customer,lead, risk_date, opportunity, html, pay, invoice, email_count, campaign):
    ...
    data_dict["testing"] = {}
    data_dict["testing"]["rmp"] = risk_date
    ...
    data_dict["RPM2"] = OrderedDict()     
    data_dict["Product Sold"] = OrderedDict()
    data_dict["Renewal"] = OrderedDict()
    data_dict["Service Expired"] = OrderedDict()
...
# adding opportunity details to data_dict    
    ...
    if invoice: 
        ...
        if len(inv_data)>0:
            for j in range(0,len(inv_data)):
                ...
                if item_name in item and price_list in duration:
                    renewal_data.append(item_name)
                    first_renewal_creation = inv_data[j].get("approved_on").strftime("%d-%m-%Y")
                else:
                    if item_name:
                        item.add(item_name)
                    if price_list:
                        duration.add(price_list)          
        
        if first_renewal_creation:
            data_dict["Creation_data"]["Renewal_creation"] = first_renewal_creation
        data_dict["Renewal"]["Count"] = len(renewal_data)
        for index, val in enumerate(renewal_data):
            data_dict["Renewal"]["Product" + str(index + 1)] = renewal_data[index]
        data_dict["Testing"]["product"] = product
        data_dict["Testing"]["item"] = inv_data
    ...
    if len(inv_data)>0:
        data_dict["Product Sold"]["Status"] = "Yes" 
    if len(inv_data)>1 and len(risk_date)>1: 
        data_dict["Creation_data"]["RPM2_creation"] = risk_date[len(risk_date)-1][1].strftime("%d-%m-%Y")
        data_dict["Creation_data"]["RPM2_diff"] = days_between(risk_date[0][1], risk_date[len(risk_date)-1][1]) 
    ...
    if len(inv_data)>0:
       if risk_date:
           if len(inv_data)>1 and len(risk_date)>1:
               data_dict["RPM2"]["Risk Appetite"] = risk_date[len(risk_date)-1][2]  
       data_dict["Sales Invoice"] = {}
       data_dict["Sales Invoice"]["type"] = "nested"
       data_dict["Sales Invoice"]["Count"] = len(inv_data)
       service_expired_date_list = []
       data_dict["Service Expired"]["type"] = "nested"
       for index, item in enumerate(inv_data):
           data_dict["Sales Invoice"][item.get("name")] = {}
           data_dict["Service Expired"][item.get("name")] = {}
	       data_dict["Sales Invoice"][item.get("name")]["id"] = item.get("name")
           if (item.get("end_date") < datetime.date.today()):
               
               data_dict["Service Expired"][item.get("name")]["id"] = item.get("name")
               service_expired_date_list.append(item.get(item)) 
	  
    
    data_dict["Service Expired"]["Count"] = len(service_expired_date_list)  
...
def customer_details(name):
    ...
    pay = frappe.db.sql("""select creation,paid_amount,bank_account,paid_to_account_currency, name from `tabPayment Entry` where title=%s and workflow_state="Approved" order by creation  """,(name))
       
    
         

    invoice = frappe.get_list("Sales Invoice", filters = {'customer': name, 'workflow_state': "Approved"}, fields = ['name'], order_by = 'creation')
